		...
		<div class="panel panel-edit">
			...
			<div class="panel-body">
				<?php
                    ...
                    d2u_addon_backend_helper::form_select('d2u_helper_translations_language', 'settings[clang_id]', $lang_options, [is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId()]);

                    $filter_options = [
                        'update' => rex_i18n::msg('d2u_helper_translations_filter_update'),
                        'missing' => rex_i18n::msg('d2u_helper_translations_filter_missing'),
                    ];
                    d2u_addon_backend_helper::form_select('d2u_helper_translations_filter_select', 'settings[filter]', $filter_options, [is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update']);
                ?>
			</div>
...
<?php
    ...
    if (rex_addon::get('d2u_address')->isAvailable()) {
        $continents = D2U_Address\Continent::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
        $countries = D2U_Address\Country::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
				...
				<?php
                    if (count($continents) > 0) {
                        ...
                        foreach ($continents as $continent) {
                            if ('' !== $continent->name) {
                                $continent = new \D2U_Address\Continent($continent->continent_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($countries) > 0) {
                        ...
                        foreach ($countries as $country) {
                            if ('' !== $country->name) {
                                $country = new \D2U_Address\Country($country->country_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_history')->isAvailable()) {
        $history_events = D2U_History\History::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
                    ...
                    if (count($history_events) > 0) {
                        ...
                        foreach ($history_events as $history_event) {
                            if ('' !== $history_event->name) {
                                $history_event = new \D2U_History\History($history_event->history_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_immo')->isAvailable()) {
        $categories = D2U_Immo\Category::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
        $properties = D2U_Immo\Property::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
		...
		<div class="panel-body">
				...
				<?php
                    if (count($categories) > 0) {
                        ...
                        foreach ($categories as $category) {
                            if ('' !== $category->name) {
                                $category = new \D2U_Immo\Category($category->category_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($properties) > 0) {
                        ...
                        foreach ($properties as $property) {
                            if ('' !== $property->name) {
                                $property = new \D2U_Immo\Property($property->property_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                if (rex_plugin::get('d2u_immo', 'window_advertising')->isAvailable()) {
                    $ads = D2U_Immo\Advertisement::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($ads) > 0) {
                        ...
                        foreach ($ads as $ad) {
                            if ('' !== $ad->title) {
                                $ad = new \D2U_Immo\Advertisement($ad->ad_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_jobs')->isAvailable()) {
        $categories = D2U_Jobs\Category::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
        $jobs = D2U_Jobs\Job::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
			...
			<fieldset>
				...
				<?php
                    if (count($categories) > 0) {
                        ...
                        foreach ($categories as $category) {
                            if ('' !== $category->name) {
                                $category = new \D2U_Jobs\Category($category->category_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_linkbox')->isAvailable()) {
        $linkboxes = \D2U_Linkbox\Linkbox::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
				...
				<?php
                    if (count($linkboxes) > 0) {
                        ...
                        foreach ($linkboxes as $linkbox) {
                            if ('' !== $linkbox->title) {
                                $linkbox = new \D2U_Linkbox\Linkbox($linkbox->box_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
        ...
        $categories = Category::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
        $machines = Machine::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
		...
		<div class="panel-body">
			<fieldset>
				...
				<?php
                    if (count($categories) > 0) {
                        ...
                        foreach ($categories as $category) {
                            if ('' !== $category->name) {
                                $category = new Category($category->category_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($machines) > 0) {
                        ...
                        foreach ($machines as $machine) {
                            if ('' !== $machine->name) {
                                $machine = new Machine($machine->machine_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                if (rex_plugin::get('d2u_machinery', 'equipment')->isAvailable()) {
                    $equipments = Equipment::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $equipment_groups = EquipmentGroup::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($equipments) > 0) {
                        ...
                        foreach ($equipments as $equipment) {
                            if ('' !== $equipment->name) {
                                $equipment = new Equipment($equipment->equipment_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
                    ...
                    if (count($equipment_groups) > 0) {
                        ...
                        foreach ($equipment_groups as $equipment_group) {
                            if ('' !== $equipment_group->name) {
                                $equipment_group = new EquipmentGroup($equipment_group->group_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'industry_sectors')->isAvailable()) {
                    $industry_sectors = IndustrySector::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
                    ...
                    if (count($industry_sectors) > 0) {
                        ...
                        foreach ($industry_sectors as $industry_sector) {
                            if ('' !== $industry_sector->name) {
                                $industry_sector = new IndustrySector($industry_sector->industry_sector_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'machine_certificates_extension')->isAvailable()) {
                    $certificates = Certificate::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($certificates) > 0) {
                        ...
                        foreach ($certificates as $certificate) {
                            if ('' !== $certificate->name) {
                                $certificate = new Certificate($certificate->certificate_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'machine_features_extension')->isAvailable()) {
                    $features = Feature::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($features) > 0) {
                        ...
                        foreach ($features as $feature) {
                            if ('' !== $feature->name) {
                                $feature = new Feature($feature->feature_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'machine_options_extension')->isAvailable()) {
                    $options = Option::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($options) > 0) {
                        ...
                        foreach ($options as $option) {
                            if ('' !== $option->name) {
                                $option = new Option($option->option_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'machine_steel_processing_extension')->isAvailable()) {
                    $automations = Automation::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $materials = Material::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $procedures = Procedure::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $processes = Process::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $profiles = Profile::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $supplies = Supply::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $tools = Tool::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
                    $weldings = Welding::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($automations) > 0) {
                        ...
                        foreach ($automations as $automation) {
                            if ('' !== $automation->name) {
                                $automation = new Automation($automation->automation_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($materials) > 0) {
                        ...
                        foreach ($materials as $material) {
                            if ('' !== $material->name) {
                                $material = new Material($material->material_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($procedures) > 0) {
                        ...
                        foreach ($procedures as $procedure) {
                            if ('' !== $procedure->name) {
                                $procedure = new Procedure($procedure->procedure_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($processes) > 0) {
                        ...
                        foreach ($processes as $process) {
                            if ('' !== $process->name) {
                                $process = new Process($process->process_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($profiles) > 0) {
                        ...
                        foreach ($profiles as $profile) {
                            if ('' !== $profile->name) {
                                $profile = new Profile($profile->profile_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($supplies) > 0) {
                        ...
                        foreach ($supplies as $supply) {
                            if ('' !== $supply->name) {
                                $supply = new Supply($supply->supply_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($tools) > 0) {
                        ...
                        foreach ($tools as $tool) {
                            if ('' !== $tool->name) {
                                $tool = new Tool($tool->tool_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
				...
				<?php
                    if (count($weldings) > 0) {
                        ...
                        foreach ($weldings as $welding) {
                            if ('' !== $welding->name) {
                                $welding = new Welding($welding->welding_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'machine_usage_area_extension')->isAvailable()) {
                    $usage_areas = UsageArea::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($usage_areas) > 0) {
                        ...
                        foreach ($usage_areas as $usage_area) {
                            if ('' !== $usage_area->name) {
                                $usage_area = new UsageArea($usage_area->usage_area_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
                    ...
                    if (count($production_lines) > 0) {
                        ...
                        foreach ($production_lines as $production_line) {
                            if ('' !== $production_line->name) {
                                $production_line = new ProductionLine($production_line->production_line_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'service_options')->isAvailable()) {
                    $service_options = ServiceOption::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
                    ...
                    if (count($service_options) > 0) {
                        ...
                        foreach ($service_options as $service_option) {
                            if ('' !== $service_option->name) {
                                $service_option = new ServiceOption($service_option->service_option_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<?php
                ...
                if (rex_plugin::get('d2u_machinery', 'used_machines')->isAvailable()) {
                    $used_machines = UsedMachine::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
                    ...
                    if (count($used_machines) > 0) {
                        ...
                        foreach ($used_machines as $used_machine) {
                            if ('' !== $used_machine->name) {
                                $used_machine = new UsedMachine($used_machine->used_machine_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_staff')->isAvailable()) {
        $staff_members = Staff::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
                    ...
                    if (count($staff_members) > 0) {
                        ...
                        foreach ($staff_members as $staff_member) {
                            if ('' !== $staff_member->name) {
                                $staff_member = new Staff($staff_member->staff_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_news')->isAvailable()) {
        $news = \D2U_News\News::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
        $news_categories = \D2U_News\Category::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
		...
		<div class="panel-body">
                    ...
                    if (count($news_categories) > 0) {
                        ...
                        foreach ($news_categories as $current_news_category) {
                            if ('' !== $current_news_category->name) {
                                $current_news_category = new \D2U_News\Category($current_news_category->category_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
                    ...
                    if (count($news) > 0) {
                        ...
                        foreach ($news as $current_news) {
                            if ('' !== $current_news->name) {
                                $current_news = new \D2U_News\News($current_news->news_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                                if ('' !== $current_news->name) {
                                    foreach (rex_clang::getAllIds() as $clang_id) {
			...
			<?php
                if (rex_plugin::get('d2u_news', 'news_types')->isAvailable()) {
                    $news_types = \D2U_News\Type::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
            ?>
			<br>
			<fieldset>
				...
				<?php
                    if (count($news_types) > 0) {
                        ...
                        foreach ($news_types as $news_type) {
                            if ('' !== $news_type->name) {
                                $news_type = new \D2U_News\Type($news_type->type_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_references')->isAvailable()) {
        $references = Reference::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
        $tags = Tag::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
				...
				<?php
                    if (count($references) > 0) {
                        ...
                        foreach ($references as $reference) {
                            if ('' !== $reference->name) {
                                $reference = new Reference($reference->reference_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
			...
			<fieldset>
				...
				<?php
                    if (count($tags) > 0) {
                        ...
                        foreach ($tags as $tag) {
                            if ('' !== $tag->name) {
                                $tag = new Tag($tag->tag_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
...
<?php
    ...
    if (rex_addon::get('d2u_videos')->isAvailable()) {
        $videos = Video::getTranslationHelperObjects(is_array(rex_session('d2u_helper_translation')) && array_key_exists('clang_id', rex_session('d2u_helper_translation')) ? (int) rex_session('d2u_helper_translation')['clang_id'] : rex_clang::getStartId(), is_array(rex_session('d2u_helper_translation')) && array_key_exists('filter', rex_session('d2u_helper_translation')) ? (string) rex_session('d2u_helper_translation')['filter'] : 'update');
?>
	<div class="panel panel-edit">
				...
				<?php
                    if (count($videos) > 0) {
                        ...
                        foreach ($videos as $video) {
                            if ('' !== $video->name) {
                                $video = new Video($video->video_id, (int) rex_config::get('d2u_helper', 'default_lang'));
                            }
