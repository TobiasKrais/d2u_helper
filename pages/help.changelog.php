<div class="panel panel-edit">
	...
	<div class="panel-body">
		<p>2.3.0-DEV</p>
		<ul>
			<li>Übersetzungshilfe (Artikel-Inhalte): Beim Aktualisieren vorhandener Slices werden jetzt alle Werte aus der Quelle übernommen — <strong>außer PDF-Medienwerten</strong>: hält eine Medien-Spalte im Ziel bereits ein <code>.pdf</code>, bleibt sie erhalten (manuell gesetztes übersetztes PDF wird nicht überschrieben). Zusätzlich eigene Spalten für Status (✓ aktuell / ≠ Übersetzung nötig) und PDF-Medien; das PDF-Symbol verlinkt auf den ersten PDF-Slice in der Ausgangssprache. Baum-Darstellung mit Ordner-/leeres-Ordner-/Artikel-Symbolen und ebenengleicher Einrückung (auch im SEO-Tab). Neuer Schalter „Seiten mit PDF-Medien überspringen" (aus Sicherheitsgründen standardmäßig aktiv) klammert PDF-Seiten aus dem Sammel-Übersetzen aus.</li>
			<li>Übersetzungshilfe (Artikel-Inhalte): Medien-Referenzen werden beim Übersetzen/Kopieren auf die Zielsprache umgebogen. Standard ist die Sprach-Suffix-Konvention (z.&nbsp;B. <code>flyer_en.pdf</code> → <code>flyer_de.pdf</code>, Suffix = Sprach-Kürzel). Betroffen sind Medien-Felder (<code>media*</code>/<code>medialist*</code>) und <code>/media/…</code>-Links in HTML-Werten. Die Ersetzung greift nur, wenn die Zieldatei im Medienpool existiert; sonst bleibt das Original erhalten. Zusätzlich markiert die Vergleichsliste Seiten mit PDF-Medien mit einem PDF-Symbol, und am Seitenende steht ein Hinweis dazu, sodass fehlende übersetzte Dokumente auffallen.</li>
			<li>Bugfix Breadcrumbs: Ein Addon-Breadcrumb (Extension Point <code>D2U_HELPER_BREADCRUMBS</code>, z.&nbsp;B. aus d2u_references) wird nicht mehr doppelt angezeigt, wenn das <code>url</code>-Addon den aktuellen Artikel bereits auf den Detail-Datensatz umbenannt hat. <code>getBreadcrumbs()</code> überspringt jetzt ein Addon-Breadcrumb, dessen Link-Ziel identisch zum vorherigen Eintrag ist.</li>
			<li>Übersetzungshilfe überarbeitet: zwei Tabs — „Redaxo Artikel Inhalte" (Standard; Redaxo-Artikel-Slices als Struktur-Tabelle mit Status je Artikel, Einzel- und Sammelübersetzung per AJAX ohne Seiten-Reload) und „D2U Addons" (fehlende/zu aktualisierende Objekte als Tabelle ohne Filter). Ist das Addon <code>ai_platform</code> mit Standard-Textprofil vorhanden, werden Übersetzungen automatisch erstellt — einzeln, je Kategorie/Spalte oder alle; die Übersetzungslogik liefern die Addons über den Extension Point <code>D2U_HELPER_TRANSLATE_OBJECT</code> (zentral neu: <code>AiTranslationHelper</code>, die Endpunkte <code>rex_api_d2u_helper_translate</code> / <code>rex_api_d2u_helper_article_translate</code>). Zusätzlich lassen sich einzelne Slices direkt im Content-Editor übersetzen (Standardmodule tragen dafür einen <code>d2u_translate</code>-Marker; Modul-Revisionen erhöht).</li>
			<li>Übersetzungshilfe: Neuer Tab „Kategorien & SEO". Listet alle Kategorien und Artikel als Struktur-Tabelle mit dem Online-Status der Zielsprache und — falls <code>yrewrite</code> installiert — SEO-Titel, SEO-Beschreibung und SEO-Bild. Die nicht übersetzbaren Daten (Online-Status, SEO-Bild) stehen links; ein „Angleichen"-Symbol erscheint direkt in der Online- bzw. Bild-Spalte, aber nur wenn es dort etwas anzugleichen gibt. Die Textfelder lassen sich per KI „Übersetzen" und der Online-Status/das Bild angleichen — je Zeile und als Sammelaktion, jeweils per AJAX (Zeile aktualisiert sich ohne Neuladen). Der Bearbeitungsbedarf richtet sich nach dem Änderungsdatum (Quelle nach der Übersetzung geändert = „veraltet"), nicht nach Textgleichheit. Die Ausgangssprache wird angezeigt und der Sprachwähler wirkt direkt bei Änderung.</li>
