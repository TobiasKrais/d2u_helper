<div class="panel panel-edit">
	...
	<div class="panel-body">
		<p>2.3.0</p>
		<ul>
			<li>Übersetzungshilfe „Redaxo Artikel Inhalte" (umbenannt, jetzt Standard-Tab): Die Artikel werden jetzt als Tabelle mit Struktur-Einrückung dargestellt (übergeordnete Ebenen immer sichtbar, Kategorien mit Ordnersymbol und Kategorienamen ein-/ausklappbar, Artikelnamen verlinkt, Tabellenkopf bleibt beim Scrollen sichtbar). Eigene Spalten zeigen den Status je Artikel — ohne Inhalt, fehlende Blöcke, zu aktualisierende Blöcke — mit der jeweiligen Anzahl direkt vor dem Aktions-Button. Pro Artikel drei Aktionen (komplett kopieren, nur fehlende ergänzen, nur veraltete aktualisieren — jeweils übersetzen), zusätzlich Mehrfachauswahl mit Sammelaktionen. Die Aktion „komplett kopieren" erscheint nur bei Artikeln ohne Zielinhalt. Der Status-Filter entfällt.</li>
			<li>Übersetzungshilfe „Redaxo Artikel Inhalte": Die Übersetzungs-Aktionen (einzeln und als Sammelaktion) laufen jetzt per AJAX — die betroffene Zeile wird direkt aktualisiert, ohne die ganze Seite neu zu laden. Neuer Endpunkt <code>rex_api_d2u_helper_article_translate</code> (Rechteprüfung + CSRF); ohne JavaScript funktionieren die Buttons weiterhin als normales Formular.</li>
			<li>Übersetzungshilfe „D2U Addons": Die zu übersetzenden Objekte werden jetzt ebenfalls als Tabelle (Addon, Bereich, Objekte) und ohne Filter angezeigt — fehlende und zu aktualisierende Übersetzungen zusammengefasst. Neben „Alle mit KI übersetzen" (übersetzt fehlende und zu aktualisierende) gibt es jetzt zwei zusätzliche Buttons „Nur fehlende" und „Nur zu aktualisierende", die gezielt nur die jeweilige Spalte übersetzen.</li>
			<li>Neu: Alle mitgelieferten D2U-Standardmodule mit übersetzbarem Textinhalt (Texteditor, Überschrift, Box, Karten-Infotext, Lauftext, Button, Kontakt-Sektion, Google-Places-Überschrift) tragen jetzt einen <code>d2u_translate</code>-Marker im Modul-Code. Dadurch erscheint im Content-Editor an den betreffenden Slices der KI-Übersetzen-Button (siehe Slice-Übersetzung). Modul-Revisionen erhöht.</li>
			<li>Neu: KI-Übersetzung in der Übersetzungshilfe. Ist das Addon <code>ai_platform</code> installiert und ein Standard-Textprofil konfiguriert, erscheint hinter jeder fehlenden oder zu aktualisierenden Übersetzung ein Icon zum automatischen Übersetzen sowie ein Button „Alle mit KI übersetzen". Die eigentliche Übersetzungslogik bleibt in den jeweiligen Addons: Über den neuen Extension Point <code>D2U_HELPER_TRANSLATE_OBJECT</code> (analog zu <code>D2U_HELPER_TRANSLATION_LIST</code>) übersetzt jedes Addon seine eigenen Objekte. Zentral neu in d2u_helper sind die Klasse <code>AiTranslationHelper</code> (Verfügbarkeitsprüfung und gebündelte Übersetzung mehrerer Felder in einem Aufruf, HTML-erhaltend), der Backend-Endpunkt <code>rex_api_d2u_helper_translate</code> (Rechteprüfung <code>d2u_helper[translation_helper]</code> und CSRF), <code>BackendHelper::getTranslationItem()</code> für einheitliche Listeneinträge mit Übersetzen-Icon sowie das zugehörige Backend-JavaScript (Einzel- und sequentielle Sammelübersetzung). Ist <code>ai_platform</code> nicht installiert, nicht aktiviert oder kein Standard-Textprofil konfiguriert, während Übersetzungen offen sind, erscheint ein Hinweis.</li>
			<li>Neu: In der Übersetzungshilfe lässt sich neben „Alle mit KI übersetzen" und der Einzelübersetzung jetzt auch jede Kategorie (Gruppe) einzeln übersetzen. Hinter jeder Kategorie-Überschrift (<code>&lt;legend&gt;</code>) steht ein Button „Kategorie mit KI übersetzen", der sequentiell nur die noch offenen Einträge dieser Kategorie übersetzt. Der Button erscheint nur, wenn die KI-Übersetzung verfügbar ist.</li>
			<li>Sprach-Wildcards / Sprog-2-Kompatibilität (<code>ALangHelper</code>): Sprog 2.0 nutzt ein neues Schema (<code>sprog_unit</code>/<code>sprog_translation</code>) mit eigener Repository-API. D2U schreibt seine Wildcards jetzt schema-abhängig: unter Sprog 2 über die v2-API (<code>UnitRepository</code>/<code>TranslationRepository</code>, Namespace <code>wildcard</code>), unter Sprog 1 weiterhin in die Tabelle <code>rex_sprog_wildcard</code>. Ist weder die v2-API noch die v1-Tabelle vorhanden, wird nichts geschrieben (keine Tabelle wird angelegt) – das behebt die <code>rex_sql_exception</code> („Table <code>rex_sprog_wildcard</code> doesn't exist") beim Speichern der Einstellungen bzw. beim (Neu-)Installieren eines D2U-Addons unter Sprog 2. <code>uninstall()</code> entfernt die Wildcards entsprechend aus dem jeweils vorhandenen Schema.</li>
			...
			<li>Bugfix Zahlen-Eingabefeld (<code>BackendHelper::form_input()</code> mit <code>type="number"</code>, z. B. das Prioritätsfeld): Bei jeder Änderung wurde ein <code>onChange</code> ausgelöst, das ein nur für Farbfelder existierendes <code>text-</code>-Eingabefeld ansprach. Ergebnis war ein <code>TypeError: document.getElementById(...) is null</code>. Das <code>onChange</code> wird jetzt ausschließlich für Farbfelder gesetzt.</li>
			<li>Farb-Doppelfeld (<code>BackendHelper::form_input_color_pair()</code>): Das Textfeld war leer, während der Color-Picker sichtbar Weiß (Light) bzw. Schwarz (Dark) vorwählte. Jetzt wird das Textfeld mit dem aufgelösten Wert vorbelegt (gespeicherter Wert bzw. <code>#ffffff</code>/<code>#000000</code>), sodass Anzeige und gespeicherter Wert übereinstimmen. Zum Zurücksetzen auf die Theme-Farbe kann das Textfeld geleert werden.</li>
			<li>Neu: Gemeinsame SEO-Hinweis-Sprachschlüssel <code>d2u_helper_seo_hint_meta_title</code>, <code>d2u_helper_seo_hint_meta_description</code>, <code>d2u_helper_seo_hint_first_image</code> und <code>d2u_helper_seo_hint_image</code>. Damit können die D2U-Addons einheitlich an den betroffenen Formularfeldern anzeigen, dass ein Feld als Meta-Titel bzw. Meta-Beschreibung oder ein Bild als SEO-/Social-Media-Vorschau (<code>og:image</code>) genutzt wird, ohne den Text je Addon zu duplizieren.</li>
		</ul>
		<p>2.2.7</p>
		<ul>
		...
		<ul>
			...
			<li>Verbesserung Backend-Einstellungen: Die Firmen-/Kontaktdaten (Firma, Geschäftsführer, Straße, PLZ/Ort, Land, Telefon, Mobil, Fax, E-Mail) bleiben jetzt unabhängig vom gewählten Footer-Typ und der CTA-Box immer editierbar, da sie auch die strukturierten Daten (JSON-LD) speisen. Das neue Feld „Länderkennung" (<code>addressCountry</code>) steht direkt bei der Adresse; das separate JSON-LD-Feld beschränkt sich auf den Schema-Typ (<code>@type</code>).</li>
		...
		<ul>
			...
			<li>Security <code>boot.php</code> (Medienpool-Verwendungsprüfung): Der in der Warnmeldung verlinkte Template-Name wird jetzt vor der Ausgabe über <code>rex_escape()</code> escaped.</li>
			<li>Security Navigations-Klassen (<code>FrontendNavigationBS5</code>, <code>FrontendNavigationMegaMenu</code>, <code>FrontendNavigationResponsiveMultiLevel</code>, <code>FrontendNavigationSlickNav</code>, <code>FrontendNavigationSmartmenu</code>): Im Frontend ausgegebene Kategorie- und Artikelnamen sowie Sprachnamen werden jetzt durchgängig über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped (Linktext, <code>title</code>- und <code>alt</code>-Attribute, Sprachumschalter-Links). Die Sprachwechsel-URLs im Sprachmodal/Dropdown werden ebenfalls per <code>html_attr</code> escaped.</li>
			<li>Security <code>BackendHelper::form_linkfield()</code> / <code>form_linklistfield()</code>: Die im Backend angezeigten Artikelnamen werden jetzt vor der Ausgabe in das <code>value</code>-Attribut bzw. die <code>&lt;option&gt;</code>-Liste über <code>rex_escape()</code> escaped.</li>
			<li>Security Backend-Modulformulare (Eingabemasken 01-2, 01-5, 03-1, 03-4, 10-1, 10-2, 11-2, 11-5, 14-1, 12-1, 12-2): Aus der Datenbank stammende Media-Manager-Typ-Namen und Feed-Titel werden in den <code>&lt;option&gt;</code>-Listen jetzt über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped.</li>
			<li>Security Module 11-2, 11-3, 11-5 (Kontaktperson / Kontaktbereich): Im Frontend ausgegebene REX_VALUE-Inhalte (Name, Straße, Ort, Telefon, E-Mail, Überschrift, Kontakt-Linktext) sowie Medien-Titel werden jetzt durchgängig über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped. In Modul 11-3 wird die als Inline-CSS verwendete Hintergrundfarbe zusätzlich über <code>BackendHelper::sanitizeHexColor()</code> validiert.</li>
			<li>Security Modul 05-1 (Weiterleitung): Die im Backend angezeigte Debug-Ausgabe (Ziel-URL, Maschinen-/Immobilien-/Kategorie-/Artikelnamen, Zusatzparameter, Anker) wird jetzt über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped.</li>
			<li>Security Footer-Templates / CTA-Box (<code>fragments/d2u_template_cta_box.php</code>, <code>d2u_template_footer*</code>, <code>d2u_template_bs5_footer*</code>): Im Frontend ausgegebene Einstellungs-Werte (Telefon, Mobil, Fax, E-Mail, Firma, Geschäftsführer, Straße, PLZ/Ort), verlinkte Social-Media-URLs, Artikelnamen sowie Medien-Titel werden jetzt durchgängig über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped. Social-Media-Links werden zusätzlich über den neuen Helper <code>FrontendHelper::sanitizeUrl()</code> auf zulässige Schemata (<code>http</code>, <code>https</code>, schemenlose bzw. relative URLs) eingeschränkt, sodass <code>javascript:</code>- und andere gefährliche Schemata nicht mehr in das <code>href</code>-Attribut gelangen. Das HTML-Freitextfeld <code>footer_text</code> bleibt bewusst unverändert.</li>
		...
		<ul>
			...
			<li>Security: Modul- und Template-Manager (<code>pages/modules.php</code>, <code>pages/templates.php</code>) validieren state-ändernde Aktionen jetzt über CSRF-Token; Action-Links und Formulare in <code>ModuleManager::showManagerList()</code> und <code>TemplateManager::showManagerList()</code> tragen das Token mit.</li>
			<li>Security: SQL-Injection-Härtung mittels Parameter-Binding in <code>BackendHelper::getMediapoolFilename()</code>, <code>BackendHelper::update_url_scheme()</code>, <code>Module::isModuleIDInstalled()</code>, <code>TemplateManager::getTemplatePairs()</code>, der Suchergebnisausgabe in Modul 14-1 sowie der Media-In-Use-Prüfung in <code>boot.php</code>.</li>
			<li>Security: Zusätzliche SQL-Injection-Härtung in <code>ALangHelper::getId()</code>, <code>ALangHelper::saveValue()</code>, <code>ALangHelper::uninstall()</code> und in den CronJob-Methoden <code>ACronJob::activate()</code>, <code>deactivate()</code>, <code>delete()</code>, <code>isInstalled()</code> sowie <code>save()</code>; Werte werden jetzt als Bind-Parameter übergeben statt per <code>addslashes</code>/Stringkonkatenation.</li>
			<li>Security: Hex-Farben aus den Einstellungen werden vor dem Speichern strikt validiert (#RGB / #RRGGBB / #RRGGBBAA). Zusätzlich validieren <code>FrontendHelper::generateCSSVariables()</code> und <code>FrontendHelper::applySettingsToCSS()</code> die Werte als Defense-in-Depth, damit ungültige Konfigurationen nicht in den ausgelieferten CSS-Block geraten. Neuer Helper <code>BackendHelper::sanitizeHexColor()</code>.</li>
			<li>Security Modul 14-1 (Search It Suchmodul): Suchtreffer-Ausgabe escaped jetzt Artikelnamen, URL-Titel, Mediatitel sowie verlinkte URLs durchgängig über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code>; das von search_it gelieferte <code>highlightedtext</code>-Markup bleibt unverändert.</li>
			<li>Security Module 04-1, 04-2, 04-3, 04-4 (Google Maps / OpenStreetMap): REX_VALUE-Werte für JS- und CSS-Kontexte werden jetzt strikt typisiert (Latitude/Longitude als float, Zoom/Höhe als int, Höheneinheit aus Whitelist <code>px|%|em|rem|vh|vw</code>), Map-Type aus fester Whitelist (<code>HYBRID|ROADMAP|SATELLITE|TERRAIN</code>), Google-API-Key per <code>rawurlencode</code> abgesichert, Adresse und Infotext via <code>json_encode</code> in JS-Strings eingefügt, Leaflet-Popup nutzt ebenfalls <code>json_encode</code> statt <code>addslashes</code>. Geolocation-positionColor wird vor der Ausgabe über <code>BackendHelper::sanitizeHexColor()</code> validiert.</li>
			<li>Security Modul 13-1 (Lauftext): Animationsdauer wird beim Einsetzen in den Inline-CSS-Wert strikt als positive Ganzzahl gecastet, damit kein CSS-Wert über die Eingabe eingeschleust werden kann.</li>
			<li>Security TOC-Generator (<code>boot.php addD2UHelperTOC</code>): Inhaltsverzeichnis wird jetzt durchgängig über DOM-API (<code>createElement</code>, <code>createTextNode</code>) aufgebaut statt per String-Konkatenation und <code>appendXML</code>; Überschriftentexte mit <code>&amp;</code> oder <code>&lt;</code> erzeugen damit kein fehlerhaftes XML mehr und werden korrekt escaped.</li>
			<li>Security Asset-Endpunkte (<code>helper.css</code>, <code>helper.js</code>, <code>template.css</code>, <code>custom.css</code>): Antworten setzen jetzt zusätzlich <code>X-Content-Type-Options: nosniff</code> sowie <code>Cache-Control: public, max-age=3600, must-revalidate</code> und liefern den Content-Type mit <code>charset=utf-8</code> aus, damit MIME-Sniffing unterbunden wird und Browser/Proxies die Auslieferung sauber zwischenspeichern.</li>
			<li>Security Custom-CSS-Einbindung (<code>boot.php sendD2UHelperCustomCSS</code> / <code>sendD2UHelperTemplateCSS</code>): Der in den Einstellungen hinterlegte Dateiname wird vor dem Einlesen jetzt strikt validiert (nur Mediapool-Dateinamen ohne Pfadanteile, Endung <code>.css</code>, vorhandener <code>rex_media</code>-Eintrag und Realpath-Prüfung gegen <code>rex_path::media('')</code>). Damit ist ein Auslesen beliebiger Dateien über manipulierte Konfigurationswerte ausgeschlossen.</li>
			<li>Security Module 02-1, 02-2, 02-3, 02-6, 02-7, 02-8 (Überschriften / Klapptexte): Der Überschriften-HTML-Tag wird vor der Ausgabe gegen eine Whitelist (<code>h1</code>–<code>h6</code>, <code>b</code>, <code>p</code>) geprüft, der Überschriftentext wird über <code>rex_escape()</code> escaped. Damit kann über manipulierte REX_VALUE-Inhalte kein eigener Tag oder Skript-Markup mehr eingeschleust werden.</li>
			<li>Security Module 06-2, 06-6 (IFrame einbinden): Die IFrame-URL wird vor der Ausgabe auf zulässige Schemata (<code>http</code>, <code>https</code>, schemenlose <code>//host</code> oder relative Pfade ab <code>/</code>) eingeschränkt und über <code>rex_escape()</code> ins <code>src</code>- bzw. <code>href</code>-Attribut eingefügt. Breite und Höhe werden als <code>int</code> gecastet, der <code>overflow</code>-Wert gegen die Whitelist <code>auto|scroll|hidden</code> validiert.</li>
			<li>Security Module 02-4, 02-9 (Überschrift mit Hintergrundbild und 2 Buttons): Überschriften-Tag jetzt gegen Whitelist (<code>h1</code>–<code>h6</code>, <code>b</code>, <code>p</code>) validiert; Überschrift, Button-Beschriftung sowie verlinkte URLs/Titel und das Hintergrundbild-<code>src</code> werden über <code>rex_escape()</code> escaped. Außerdem CSS-Fix: Der Wrapper bekommt <code>position: relative</code> und das Hintergrundbild wird auf volle Breite gesetzt, damit der Content-Layer korrekt über dem Bild liegt und nicht weiter oben auf der Seite gerendert wird.</li>
			<li>Security Module 12-1, 12-2 (Feeds Stream Galerie): Stream-/Item-Titel und Item-URLs aus externen Feeds werden jetzt durchgängig über <code>rex_escape(..., 'html_attr')</code> bzw. <code>rex_escape()</code> escaped (inkl. <code>data-title</code>, <code>data-footer-url</code>, <code>data-footer-text</code>, <code>data-footer</code> sowie <code>img</code>-<code>alt</code>/<code>title</code>); externe Feed-Inhalte können dadurch keinen ungewollten Markup mehr in das Galerie-Layout einschleusen.</li>
			<li>Security Module 15-1, 15-2 (Kategorie mit Liste der Unterkategorien): Kategorie-Namen und URLs werden vor der Ausgabe in <code>&lt;a href&gt;</code>, <code>title</code> und Linktext über <code>rex_escape()</code> escaped, ebenso die <code>&lt;h1&gt;</code>-Top-Kategorie-Überschrift.</li>
			<li>Security Module 03-1, 03-4 (Bild): Bild-URL, <code>alt</code>, <code>title</code> und Bildunterschrift werden über <code>rex_escape()</code> escaped.</li>
			<li>Security Module 03-2, 03-5 (Bildergalerie Lightbox): Detail- und Thumbnail-URLs sowie Medien-Titel (<code>data-title</code>, <code>alt</code>, <code>title</code>) werden über <code>rex_escape()</code> escaped; Lightbox-IDs als <code>int</code> gecastet. Im Inline-<code>onclick</code> wird der Galerie-Name jetzt JS-escaped.</li>
			<li>Security Module 03-3, 03-6 (360° Bild): Panorama-URL im Inline-Script wird über <code>rex_escape(..., 'js')</code> escaped, Viewer-IDs als <code>int</code> gecastet.</li>
			<li>Security Module 00-1 (Umbruch ganze Breite): Optionaler CSS-Klassenname aus REX_VALUE wird vor der Ausgabe über <code>rex_escape(..., 'html_attr')</code> escaped.</li>
			<li>Security Module 01-1, 01-2, 01-4, 01-5 (Texteditor / Texteditor mit Bild): Überschriften-Tag jetzt gegen Whitelist (<code>h1</code>–<code>h6</code>, <code>b</code>, <code>p</code>) validiert; Überschrift, Bild-URL, <code>alt</code>/<code>title</code>, Bildunterschrift und Link-URL werden über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped. Show-more/less-Buttontexte werden nicht mehr über <code>addslashes</code>, sondern über <code>rex_escape(..., 'js')</code> in den Inline-Script-Kontext eingefügt.</li>
			<li>Security Module 01-3, 01-6 (Texteditor in Alertbox): Alert-Typ wird gegen Bootstrap-Whitelist (<code>primary|secondary|success|danger|warning|info|light|dark</code>) validiert, Default <code>info</code>.</li>
			<li>Security Module 06-1, 06-5 (YouTube Video): YouTube-Video-ID wird vor der Verwendung in HTML-Attributen, IDs und Inline-CSS auf das sichere Zeichenset <code>[A-Za-z0-9_-]</code> beschränkt; Videotitel aus der oEmbed-Antwort wird über <code>rex_escape()</code> escaped.</li>
			<li>Security Module 06-3, 06-7 (Video Vidstack): JSON-LD-Felder <code>contentUrl</code>, <code>thumbnailUrl</code> und <code>name</code> werden jetzt durchgängig über <code>json_encode</code> serialisiert statt per String-Konkatenation, damit Server- oder Mediatitel keine ungewollten Steuerzeichen in den Strukturierte-Daten-Block einschleusen können.</li>
			<li>Security Module 10-1, 10-2 (Box mit Bild): Überschriften-Tag gegen Whitelist validiert; Bild-<code>src</code>, <code>alt</code>, <code>title</code> sowie die Spaltenbreiten als <code>int</code> escaped/gecastet.</li>
			<li>Security Module 10-3, 10-4 (Box mit Downloads): Überschrift, Download-Titel, Dateityp, Download-URL und Vorschaubild-<code>src</code> werden über <code>rex_escape()</code> bzw. <code>rex_escape(..., 'html_attr')</code> escaped.</li>
			<li>Security Modul 14-1 (Search It Suchmodul): Ähnliche-Wörter-Liste (Similar Words) escaped jetzt Wort-Text und URL der Folgesuche über <code>rex_escape()</code>.</li>
			<li>Modul 14-1 (Search It Suchmodul): Das Autocomplete-Plugin <code>$.fn.suggest</code> wurde durch eine jQuery-freie Vanilla-JS-Implementierung (<code>D2UHelperSearchSuggest.init()</code>) ersetzt. Damit funktioniert die Live-Suche auch in Templates, die kein jQuery mehr einbinden, und die Ergebnisliste escaped Server-Antworten zusätzlich gegen HTML-Injection.</li>
			<li>Bugfix Modul 02-10 (Inhaltsverzeichnis der Überschriften, BS5): Beim Aufklappen wurde das Inhaltsverzeichnis nicht angezeigt, weil die Toggle-Funktion nur den Inline-<code>style.display</code> ausgewertet hat, das initiale <code>display: none</code> aber aus dem Stylesheet kommt. Die Toggle-Funktion liest jetzt den Computed-Style und schaltet zwischen <code>block</code> und <code>none</code> um.</li>
			<li>Modul 02-9 (Überschrift mit Hintergrundbild und 2 Buttons, BS5): Eigene CSS-Klassen-Präfixe <code>d2u_helper_mod_02_9_*</code> statt der bisher mit Modul 02-4 geteilten Präfixe; verhindert versehentliche Style-Kollisionen, wenn beide Module auf einer Seite verwendet werden.</li>
			<li>BackendHelper um gemeinsame CSRF-Helfer erweitert; D2U-Addons können damit Formular-Submits sowie mutierende Backend-Links konsistent mit Token absichern.</li>
			<li>Bugfix Templates 00-2, 01-2, 02-2, 02-3, 03-3, 04-4 und 05-2: Breadcrumb-Links der BS5-Templates verwenden jetzt wieder konsistent die konfigurierte Titel-/Breadcrumb-Schriftfarbe statt später von Artikel- oder Navigations-Linkfarben überschrieben zu werden.</li>
			<li>Bugfix Templates 04-3 und 04-4 sowie Einstellungen: Die optionale News-Anbindung nutzt jetzt den neuen Namespace <code>TobiasKrais\D2UNews</code>; Paketkonflikte für d2u_news und d2u_courses wurden entsprechend angehoben.</li>
		</ul>
		<p>2.2.1</p>
		<ul>
		...
		<ul>
			...
			<li>Kompatibilität d2u_immo >=1.4.0 hergestellt.</li>
		...
		<ul>
			...
			<li>Module
				<ul>
					...
					<li>Module 06-3 und 06-4: Bootstrap-4 Videomodule erkennen jetzt automatisch Vidstack oder Plyr. Vidstack wird bevorzugt, Plyr bleibt als Fallback erhalten. Die Eingabemasken passen sich an den verfügbaren Player an.</li>
					...
					<li>Modul 07-1 "JavaScript einbinden": Vereinfacht auf reines JavaScript-Textfeld (Eingabe) und reine JavaScript-Ausgabe. Spaltenbreiten und JS-Datei-Optionen entfernt. BS4 und BS5 zusammengeführt, Modul 07-2 entfernt.</li>
			...
			<li>Kompatibilität und Integrationen
				<ul>
					<li>Anpassungen an Search It 7.x: Search-Modul und Such-Icon nutzen jetzt die namespaced Klasse FriendsOfRedaxo\SearchIt\SearchIt sowie den neuen Autocomplete-Endpunkt search_it_autocomplete.</li>
		...
		<ul>
			<li>Folgende Klassen werden ab dieser umbenannt und die alten Klassennamen stehen nicht mehr zur Verfügung:
				<ul>
					...
					<li><code>D2U_Helper\ITranslationHelper</code> wird zu <code>TobiasKrais\D2UHelper\ITranslationHelper</code>.</li>
					<li><code>d2u_addon_backend_helper</code> wird zu <code>TobiasKrais\D2UHelper\BackendHelper</code>.</li>
					<li><code>d2u_addon_frontend_helper</code> wird zu <code>TobiasKrais\D2UHelper\FrontendHelper</code>.</li>
					<li><code>d2u_helper_lang_helper</code> wird zu <code>TobiasKrais\D2UHelper\LangHelper</code>.</li>
					<li><code>d2u_mobile_navi</code> wird zu <code>TobiasKrais\D2UHelper\FrontendNavigationResponsiveMultiLevel</code>.</li>
					<li><code>d2u_mobile_navi_mega_menu</code> wird zu <code>TobiasKrais\D2UHelper\FrontendNavigationMegaMenu</code>.</li>
					<li><code>d2u_mobile_navi_slicknav</code> wird zu <code>TobiasKrais\D2UHelper\FrontendNavigationSlickNav</code>.</li>
					<li><code>d2u_mobile_navi_smartmenus</code> wird zu <code>TobiasKrais\D2UHelper\FrontendNavigationSmartmenu</code>.</li>
					<li><code>D2UModule</code> wird zu <code>TobiasKrais\D2UHelper\Module</code>.</li>
					<li><code>D2UModuleManager</code> wird zu <code>TobiasKrais\D2UHelper\ModuleManager</code>.</li>
					<li><code>D2UTemplate</code> wird zu <code>TobiasKrais\D2UHelper\Template</code>.</li>
		...
		<ul>
			...
			<li>Modul 04-2 "OpenStreetMap Karte": Unterstützt nur auch Karten des Geolocation Addons Version 2.</li>
		...
		<ul>
			...
			<li>Modul 02-5 "Inhaltsverzeichnis der Überschriften" hinzugefügt. Dieses Modul liest alle Überschriften aus der Seite aus und fügt ein verlinktes Inhaltsverzeichnis zu den Überschriften ein.</li>
		...
		<ul>
			...
			<li>Fragment für Footer/CTA Box: Telefonnummern mit 'tel:' Links versehen.</li>
			...
			<li>Modul 11-1 "YForm Kontaktformular (DSGVO kompatibel)" YForm Template Fehler behoben.</li>
		...
		<ul>
			...
			<li>Templates 05-1 und 06-1: Anpassungen an Sprachwähler.</li>
		</ul>
		<p>1.9.0</p>
		<ul>
