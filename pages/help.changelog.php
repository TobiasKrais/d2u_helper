<div class="panel panel-edit">
	...
	<div class="panel-body">
		<p>2.2.2-DEV</p>
		<ul>
			<li>Security: Modul- und Template-Manager (<code>pages/modules.php</code>, <code>pages/templates.php</code>) validieren state-aendernde Aktionen jetzt ueber CSRF-Token; Action-Links und Formulare in <code>ModuleManager::showManagerList()</code> und <code>TemplateManager::showManagerList()</code> tragen das Token mit.</li>
			<li>Security: SQL-Injection-Haertung mittels Parameter-Binding in <code>BackendHelper::getMediapoolFilename()</code>, <code>BackendHelper::update_url_scheme()</code>, <code>Module::isModuleIDInstalled()</code>, <code>TemplateManager::getTemplatePairs()</code>, der Suchergebnisausgabe in Modul 14-1 sowie der Media-In-Use-Pruefung in <code>boot.php</code>.</li>
			<li>Security: Hex-Farben aus den Einstellungen werden vor dem Speichern strikt validiert (#RGB / #RRGGBB / #RRGGBBAA). Zusaetzlich validieren <code>FrontendHelper::generateCSSVariables()</code> und <code>FrontendHelper::applySettingsToCSS()</code> die Werte als Defense-in-Depth, damit ungueltige Konfigurationen nicht in den ausgelieferten CSS-Block geraten. Neuer Helper <code>BackendHelper::sanitizeHexColor()</code>.</li>
