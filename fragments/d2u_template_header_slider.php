<?php 
	...
	$slider_pics_unfiltered = preg_grep('/^\s*$/s', explode(",", strval($d2u_helper->getConfig('template_04_header_slider_pics_clang_'. rex_clang::getCurrentId()))), PREG_GREP_INVERT);
...
<header>
			...
			if(count($slider_pics) === 1) {
				...
				for($i = 0; $i < count($slider_pics); $i++) {
					...
					if($rex_media_slider_pic instanceof rex_media) {
						...
						print '<img class="d-block w-100"'. $srcset .' src="'. ($d2u_helper->getConfig('template_header_media_manager_type', '') !== '' ? rex_media_manager::getUrl(strval($d2u_helper->getConfig('template_header_media_manager_type', '')), $slider_pics[$i]) : rex_url::media($slider_pics[$i]))
	...
	<div class="container">
		<div class="row d-print-none">
			<div class="col-12">
				<?php
					...
					if(count($slider_pics) === 1) {
						$srcset = $media_manager_webp_exists ? ' srcset="'. rex_media_manager::getUrl($media_manager_webp_type_name, $slider_pics[0]) .' 2000w"' : '';
						print '<img'. $srcset .' src="'. ($d2u_helper->getConfig('template_header_media_manager_type', '') !== '' ? rex_media_manager::getUrl(strval($d2u_helper->getConfig('template_header_media_manager_type', '')), $slider_pics[0]) : rex_url::media($slider_pics[0]))
						...
						for($k = 0; $k < count($slider_pics); $k++) {
							...
							$slider_pic = $d2u_helper->getConfig('template_header_media_manager_type', '') !== '' ? rex_media_manager::create(strval($d2u_helper->getConfig('template_header_media_manager_type')), $slider_pics[$k])->getMedia() : rex_media::get($slider_pics[$k]);
							if($rex_media_slider_pic instanceof rex_media && ($slider_pic instanceof rex_media || $slider_pic instanceof rex_managed_media)) {
								...
								// Image
								$ratio = intval($slider_pic->getWidth()) / intval($slider_pic->getHeight());
								$min_height = intval($slider_pic->getHeight()) < 250 ? intval($slider_pic->getHeight()) : 250;
								...
								print '<img class="d-block w-100"'. $srcset .'  src="'. ($d2u_helper->getConfig('template_header_media_manager_type', '') !== '' ? rex_media_manager::getUrl(strval($d2u_helper->getConfig('template_header_media_manager_type')), $slider_pics[$k]) : rex_url::media($slider_pics[$k]))
								...
								$slogan_text = strval($article instanceof rex_article && $article->getValue('art_slogan') !== '' ? $article->getValue('art_slogan') : $d2u_helper->getConfig('template_04_1_slider_slogan_clang_'. rex_clang::getCurrentId()));
						...
						if(boolval($d2u_helper->getConfig("template_04_header_slider_pics_full_width", false))) {
		...
		if(boolval($d2u_helper->getConfig("template_04_header_slider_pics_full_width", false)) === false) {
			// START Only if slider background slider is shown
				?>
			</div>
		</div>
