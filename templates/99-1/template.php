...
$rss->syndicationURL = "http://www.dailyphp.net/". $_SERVER['REQUEST_URI'];
...
class HtmlDescribable
{
     ...
     */
    public function getValue($description)
    {
        $descriptionField = new FeedHtmlField($description);
...
class FeedCreator extends HtmlDescribable
{
    ...
    public function _createStylesheetReferences()
    {
        ...
        if ($this->cssStyleSheet) {
            $xml .= '<?xml-stylesheet href="'.$this->cssStyleSheet.'" type="text/css"?>'. PHP_EOL;
        }
        if ($this->xslStyleSheet) {
            $xml .= '<?xml-stylesheet href="'.$this->xslStyleSheet.'" type="text/xsl"?>'. PHP_EOL;
        }
        return $xml;
    }
    ...
    /**
     * Generate a filename for the feed cache file. The result will be $_SERVER['REQUEST_URI'] with the extension changed to .xml.
     * For example:.
     *
...
class FeedDate
{
     ...
     * Accepts unix time stamps.
     * @param mixed $dateString optional the date this FeedDate will represent. If not specified, the current date and time is used.
     */
    public function __construct($dateString = 0)
    {
    ...
    public function createFeed()
    {
        ...
        for ($i = 0; $i < count($this->items); ++$i) {
            ...
            if (null != $this->items[$i]->date) {
                $itemDate = new FeedDate(strtotime($this->items[$i]->date));
...
class RSSCreator091 extends FeedCreator
{
    ...
    public function createFeed()
    {
        ...
        $feed .= '        <description>'.$this->getValue('description')."</description>\n";
        $feed .= '        <link>'.$this->link."</link>\n";
        $now = new FeedDate(time());
        ...
        if (null != $this->image) {
            ...
            if ('' != $this->image->description) {
                $feed .= '            <description>'.$this->image->getValue('description')."</description>\n";
            }
            $feed .= "        </image>\n";
        }
        if ('' != $this->language) {
        ...
        for ($i = 0; $i < count($this->items); ++$i) {
            ...
            $feed .= '            <link>'.htmlspecialchars($this->items[$i]->link)."</link>\n";
            $feed .= '            <description>'.$this->items[$i]->getValue('description')."</description>\n";
...
class PIECreator01 extends FeedCreator
{
    ...
    public function createFeed()
    {
        ...
        $feed .= '    <title>'.FeedCreator::iTrunc(htmlspecialchars($this->title), 100)."</title>\n";
        $this->truncSize = 500;
        $feed .= '    <subtitle>'.$this->getValue('description')."</subtitle>\n";
        $feed .= '    <link>'.$this->link."</link>\n";
        for ($i = 0; $i < count($this->items); ++$i) {
            $feed .= "    <entry>\n";
            $feed .= '        <title>'.FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)), 100)."</title>\n";
            ...
            $feed .= '            <div xmlns="http://www.w3.org/1999/xhtml">'.$this->items[$i]->getValue('description')."</div>\n";
            $feed .= "        </content>\n";
            $feed .= "    </entry>\n";
        }
        $feed .= "</feed>\n";
...
class MBOXCreator extends FeedCreator
{
    ...
    public function createFeed()
    {
        for ($i = 0; $i < count($this->items); ++$i) {
            ...
            $feed .= 'Content-Type: text/plain;'. PHP_EOL;
            $feed .= '	charset="'.$this->encoding."\"\n";
            $feed .= "Content-Transfer-Encoding: quoted-printable\n";
            $feed .= "Content-Type: text/plain\n";
            $feed .= 'From: "'.self::qp_enc($from).'"'. PHP_EOL;
            $feed .= 'Date: '.$itemDate->rfc822() . PHP_EOL;
            $feed .= 'Subject: '.self::qp_enc(FeedCreator::iTrunc($this->items[$i]->title, 100)) . PHP_EOL . PHP_EOL;
            $body = chunk_split(self::qp_enc($this->items[$i]->description));
            $feed .= preg_replace("~\nFrom ([^\n]*)(\n?)~", "\n>From $1$2\n", $body);
            $feed .= PHP_EOL . PHP_EOL;
        }
        return $feed;
    }
    ...
    public function createFeed()
    {
        ...
        if ($this->getValue('description')) {
            $feedArray[] = "<div class='".$this->stylePrefix."description'>".
                str_replace(']]>', '', str_replace('<![CDATA[', '', $this->getValue('description'))).
                '</div>';
        }
        ...
        for ($i = 0; $i < count($this->items); ++$i) {
            ...
            if ($this->items[$i]->getValue('description')) {
                $feedArray[] =
                "<div class='".$this->stylePrefix."item_description'>".
                    str_replace(']]>', '', str_replace('<![CDATA[', '', $this->items[$i]->getValue('description'))).
                    '</div>';
            }
        }
        if ($this->footer) {
...
$rss->link = "http://www.dailyphp.net/news";
$rss->feedURL = "http://www.dailyphp.net/". $_SERVER['REQUEST_URI'];
...
//
//	Beginn des REDAXO spezifischen Teiles
//
//

$rdfValidFeedTypes = ['RSS0.91', 'RSS1.0', 'RSS2.0', 'OPML', 'ATOM', 'ATOM0.3'];
if ('' != filter_input(INPUT_GET, 'rdfType') && array_search(filter_input(INPUT_GET, 'rdfType'), $rdfValidFeedTypes)) {
    $rdfType = filter_input(INPUT_GET, 'rdfType');
} else {
    $rdfType = 'RSS1.0';
}

if ('' != filter_input(INPUT_GET, 'rdfPath')) {
    $rdfPath = filter_input(INPUT_GET, 'rdfPath');
} else {
    $rdfPath = '|';
}
...
$rdfFeedItems = $sqlcon->getArray($sql);
if (!isset($rdfFeedItems)) {
    $rdfFeedItems = [];
}
reset($rdfFeedItems);
...
while ([$k, $rdfFeedItem] = each($rdfFeedItems)) {
    $item = new FeedItem();
...
echo $rss->saveFeed($rdfType, $rdfFile);
